-- =====================================================================
-- meet-again DDL (MariaDB)
-- 소스 오브 트루스: infrastructure/storage/ma-db-core 의 Exposed Table 정의.
-- Table 정의(컬럼/인덱스)를 변경하면 이 파일도 함께 갱신한다.
-- =====================================================================

-- MEMBERS
CREATE TABLE MEMBERS
(
    MEMBER_ID               BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- MemberTable 특화 컬럼들
    EMAIL                   VARCHAR(255) NOT NULL,
    PASSWORD                VARCHAR(255) NOT NULL,
    NICKNAME                VARCHAR(255) NOT NULL,
    GENDER                  VARCHAR(32)  NOT NULL,
    PHONE_NUMBER            VARCHAR(255) NOT NULL,
    NAME                    VARCHAR(255) NOT NULL,
    BIRTH_DATE              DATE         NOT NULL,
    REGION                  VARCHAR(255) NOT NULL,
    HIGH_SCHOOL             VARCHAR(255) NULL,
    UNIVERSITY              VARCHAR(255) NULL,
    PROFILE_IMAGE_URL       VARCHAR(255) NULL,
    WITHDRAWAL_REQUESTED_AT DATETIME     NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE            DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY              VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE      DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY        VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED                 BOOLEAN      DEFAULT FALSE,
    DELETED_DATE            DATETIME     NULL,
    DELETED_BY              VARCHAR(255) NULL,

    -- 인덱스
    UNIQUE INDEX idx_member_email (EMAIL),
    INDEX idx_member_phone_number (PHONE_NUMBER),
    INDEX idx_member_withdrawal_requested_at (WITHDRAWAL_REQUESTED_AT)
);

-- REFRESH TOKENS
CREATE TABLE REFRESH_TOKENS
(
    REFRESH_TOKEN_ID   BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- RefreshTokenTable 특화 컬럼들
    MEMBER_ID          BIGINT        NOT NULL,
    EXPIRATION_DATE    DATETIME      NOT NULL,
    TOKEN              VARCHAR(1024) NOT NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE       DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY         VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY   VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED            BOOLEAN      DEFAULT FALSE,
    DELETED_DATE       DATETIME     NULL,
    DELETED_BY         VARCHAR(255) NULL,

    -- 인덱스
    UNIQUE INDEX idx_refresh_token_member_id (MEMBER_ID)
);

-- TARGET INFOS
CREATE TABLE TARGET_INFOS
(
    TARGET_INFO_ID     BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- TargetInfoTable 특화 컬럼들
    REGISTER_ID        BIGINT       NOT NULL,
    NAME               VARCHAR(255) NOT NULL,
    TARGET_GENDER      VARCHAR(32)  NOT NULL,
    MIDDLE_NUMBER      VARCHAR(255) NULL,
    LAST_NUMBER        VARCHAR(255) NULL,
    `YEAR`             INT          NULL,
    `MONTH`            INT          NULL,
    `DAY`              INT          NULL,
    REGION             VARCHAR(255) NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE       DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY         VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY   VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED            BOOLEAN      DEFAULT FALSE,
    DELETED_DATE       DATETIME     NULL,
    DELETED_BY         VARCHAR(255) NULL,

    -- 인덱스
    INDEX idx_target_info_register_id (REGISTER_ID)
);

-- MATCHING RESULTS
CREATE TABLE MATCHING_RESULTS
(
    MATCHING_RESULT_ID    BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- MatchingResultTable 특화 컬럼들
    REGISTER_ID           BIGINT       NOT NULL,
    TARGET_INFO_ID        BIGINT       NOT NULL,
    TARGET_ID             BIGINT       NOT NULL,
    MIDDLE_NUMBER_MATCHED BOOLEAN      NOT NULL,
    LAST_NUMBER_MATCHED   BOOLEAN      NOT NULL,
    YEAR_MATCHED          BOOLEAN      NOT NULL,
    MONTH_MATCHED         BOOLEAN      NOT NULL,
    DAY_MATCHED           BOOLEAN      NOT NULL,
    REGION_MATCHED        BOOLEAN      NOT NULL,
    SHOWING_EXPIRY_DATE   DATETIME     NOT NULL,
    MATCHING_EXPIRY_DATE  DATE         NOT NULL,
    EXCLUDED              BOOLEAN      DEFAULT FALSE,
    CLAIM_STATUS          VARCHAR(20)  NOT NULL DEFAULT 'NONE',

    -- BaseTable 공통 컬럼들
    CREATED_DATE          DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY            VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE    DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY      VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED               BOOLEAN      DEFAULT FALSE,
    DELETED_DATE          DATETIME     NULL,
    DELETED_BY            VARCHAR(255) NULL,

    -- 인덱스
    INDEX idx_matching_register_id (REGISTER_ID),
    INDEX idx_matching_target_info_id (TARGET_INFO_ID),
    INDEX idx_matching_target_id_claim_status (TARGET_ID, CLAIM_STATUS)
);

-- MEMBER PHOTOS
CREATE TABLE MEMBER_PHOTOS
(
    MEMBER_PHOTO_ID    BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- MemberPhotoTable 특화 컬럼들
    MEMBER_ID          BIGINT       NOT NULL,
    FILE_PATH          VARCHAR(512) NOT NULL,
    ORIGINAL_FILE_NAME VARCHAR(255) NOT NULL,
    APPROVAL_STATUS    VARCHAR(32)  NOT NULL DEFAULT 'PENDING',
    THUMBNAIL_PATH     VARCHAR(512) NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE       DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY         VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY   VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED            BOOLEAN      DEFAULT FALSE,
    DELETED_DATE       DATETIME     NULL,
    DELETED_BY         VARCHAR(255) NULL,

    -- 인덱스
    INDEX idx_member_photo_member_id (MEMBER_ID)
);

-- COMMUNITY POSTS
CREATE TABLE COMMUNITY_POSTS
(
    COMMUNITY_POST_ID  BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- PostTable 특화 컬럼들
    AUTHOR_ID          BIGINT       NOT NULL,
    CATEGORY           VARCHAR(32)  NOT NULL,
    TITLE              VARCHAR(100) NOT NULL,
    CONTENT            TEXT         NOT NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE       DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY         VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY   VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED            BOOLEAN      DEFAULT FALSE,
    DELETED_DATE       DATETIME     NULL,
    DELETED_BY         VARCHAR(255) NULL
);

-- COMMUNITY COMMENTS
CREATE TABLE COMMUNITY_COMMENTS
(
    COMMUNITY_COMMENT_ID BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- CommentTable 특화 컬럼들
    POST_ID              BIGINT       NOT NULL,
    AUTHOR_ID            BIGINT       NOT NULL,
    CONTENT              TEXT         NOT NULL,
    PARENT_COMMENT_ID    BIGINT       NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE         DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY           VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE   DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY     VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED              BOOLEAN      DEFAULT FALSE,
    DELETED_DATE         DATETIME     NULL,
    DELETED_BY           VARCHAR(255) NULL,

    -- 인덱스
    INDEX idx_community_comment_post_id (POST_ID),
    INDEX idx_community_comment_parent_id (PARENT_COMMENT_ID)
);

-- COMMUNITY POST LIKES
CREATE TABLE COMMUNITY_POST_LIKES
(
    COMMUNITY_POST_LIKE_ID BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- PostLikeTable 특화 컬럼들
    POST_ID                BIGINT       NOT NULL,
    MEMBER_ID              BIGINT       NOT NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE           DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY             VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE     DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY       VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED                BOOLEAN      DEFAULT FALSE,
    DELETED_DATE           DATETIME     NULL,
    DELETED_BY             VARCHAR(255) NULL,

    -- 인덱스
    UNIQUE INDEX idx_post_like_post_id_member_id (POST_ID, MEMBER_ID)
);

-- COMMUNITY COMMENT LIKES
CREATE TABLE COMMUNITY_COMMENT_LIKES
(
    COMMUNITY_COMMENT_LIKE_ID BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- CommentLikeTable 특화 컬럼들
    COMMENT_ID                BIGINT       NOT NULL,
    MEMBER_ID                 BIGINT       NOT NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE              DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY                VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE        DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY          VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED                   BOOLEAN      DEFAULT FALSE,
    DELETED_DATE              DATETIME     NULL,
    DELETED_BY                VARCHAR(255) NULL,

    -- 인덱스
    UNIQUE INDEX idx_comment_like_comment_id_member_id (COMMENT_ID, MEMBER_ID)
);

-- INQUIRIES
CREATE TABLE INQUIRIES
(
    INQUIRY_ID         BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- InquiryTable 특화 컬럼들
    AUTHOR_ID          BIGINT        NOT NULL,
    TITLE              VARCHAR(100)  NOT NULL,
    CONTENT            VARCHAR(1500) NOT NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE       DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY         VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY   VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED            BOOLEAN      DEFAULT FALSE,
    DELETED_DATE       DATETIME     NULL,
    DELETED_BY         VARCHAR(255) NULL
);

-- POINT PRODUCTS
CREATE TABLE POINT_PRODUCTS
(
    POINT_PRODUCT_ID   BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- PointProductTable 특화 컬럼들
    NAME               VARCHAR(100) NOT NULL,
    QUANTITY           INT          NOT NULL,
    PRICE              INT          NOT NULL,
    DISPLAY_ORDER      INT          NOT NULL,
    DISCOUNT_POLICY_ID BIGINT       NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE       DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY         VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY   VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED            BOOLEAN      DEFAULT FALSE,
    DELETED_DATE       DATETIME     NULL,
    DELETED_BY         VARCHAR(255) NULL
);

-- DISCOUNT POLICIES
CREATE TABLE DISCOUNT_POLICIES
(
    DISCOUNT_POLICY_ID BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- DiscountPolicyTable 특화 컬럼들
    POLICY_TYPE        VARCHAR(20)  NOT NULL,
    START_DATE         DATE         NOT NULL,
    END_DATE           DATE         NOT NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE       DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY         VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY   VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED            BOOLEAN      DEFAULT FALSE,
    DELETED_DATE       DATETIME     NULL,
    DELETED_BY         VARCHAR(255) NULL
);

-- AMOUNT DISCOUNT POLICIES
CREATE TABLE AMOUNT_DISCOUNT_POLICIES
(
    DISCOUNT_POLICY_ID BIGINT PRIMARY KEY,
    DISCOUNT_AMOUNT    INT    NOT NULL
);

-- PERCENT DISCOUNT POLICIES
CREATE TABLE PERCENT_DISCOUNT_POLICIES
(
    DISCOUNT_POLICY_ID BIGINT PRIMARY KEY,
    DISCOUNT_PERCENT   INT    NOT NULL
);

-- XROOMS
CREATE TABLE XROOMS
(
    XROOM_ID           BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- XroomTable 특화 컬럼들
    OWNER_ID           BIGINT        NOT NULL,
    TARGET_INFO_ID     BIGINT        NOT NULL,
    TEMPLATE           VARCHAR(32)   NOT NULL,
    TITLE              VARCHAR(100)  NOT NULL,
    FINAL_MESSAGE      VARCHAR(1000) NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE       DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY         VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY   VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED            BOOLEAN      DEFAULT FALSE,
    DELETED_DATE       DATETIME     NULL,
    DELETED_BY         VARCHAR(255) NULL,

    -- 인덱스
    INDEX idx_xroom_owner_id (OWNER_ID),
    INDEX idx_xroom_target_info_id (TARGET_INFO_ID)
);

-- MEMORIES
CREATE TABLE MEMORIES
(
    MEMORY_ID            BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- MemoryTable 특화 컬럼들
    ROOM_ID              BIGINT       NOT NULL,
    TITLE                VARCHAR(200) NOT NULL,
    EVENT_DATE           DATE         NOT NULL,
    EVENT_DATE_PRECISION VARCHAR(8)   NOT NULL,
    LOCATION             VARCHAR(200) NULL,
    TEXT                 TEXT         NULL,
    LETTER               TEXT         NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE         DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY           VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE   DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY     VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED              BOOLEAN      DEFAULT FALSE,
    DELETED_DATE         DATETIME     NULL,
    DELETED_BY           VARCHAR(255) NULL,

    -- 인덱스
    INDEX idx_memory_room_id (ROOM_ID)
);

-- MEMORY EMOTION TAGS
CREATE TABLE MEMORY_EMOTION_TAGS
(
    TAG_ID             BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- MemoryEmotionTagTable 특화 컬럼들
    MEMORY_ID          BIGINT       NOT NULL,
    TAG                VARCHAR(50)  NOT NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE       DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY         VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY   VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED            BOOLEAN      DEFAULT FALSE,
    DELETED_DATE       DATETIME     NULL,
    DELETED_BY         VARCHAR(255) NULL,

    -- 인덱스
    INDEX idx_emotion_tag_memory_id (MEMORY_ID)
);

-- MEMORY MEDIA
CREATE TABLE MEMORY_MEDIA
(
    MEDIA_ID           BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- MemoryMediaTable 특화 컬럼들
    MEMORY_ID          BIGINT       NOT NULL,
    STORAGE_KEY        VARCHAR(512) NOT NULL,
    ORIGINAL_FILENAME  VARCHAR(255) NOT NULL,
    MIME_TYPE          VARCHAR(100) NOT NULL,
    FILE_SIZE          BIGINT       NOT NULL,
    THUMBNAIL_KEY      VARCHAR(512) NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE       DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY         VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY   VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED            BOOLEAN      DEFAULT FALSE,
    DELETED_DATE       DATETIME     NULL,
    DELETED_BY         VARCHAR(255) NULL,

    -- 인덱스
    INDEX idx_media_memory_id (MEMORY_ID)
);

-- MEMBER POINTS
CREATE TABLE MEMBER_POINTS
(
    MEMBER_POINT_ID    BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- MemberPointTable 특화 컬럼들
    OWNER_ID           BIGINT       NOT NULL,
    BALANCE            INT          NOT NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE       DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY         VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY   VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED            BOOLEAN      DEFAULT FALSE,
    DELETED_DATE       DATETIME     NULL,
    DELETED_BY         VARCHAR(255) NULL,

    -- 인덱스
    UNIQUE INDEX idx_member_point_owner_id (OWNER_ID)
);

-- POINT HISTORIES
CREATE TABLE POINT_HISTORIES
(
    POINT_HISTORY_ID   BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- PointHistoryTable 특화 컬럼들
    OWNER_ID           BIGINT       NOT NULL,
    POINT_PRODUCT_ID   BIGINT       NULL,
    HISTORY_TYPE       VARCHAR(16)  NOT NULL,
    QUANTITY           INT          NOT NULL,
    PAID_AMOUNT        INT          NOT NULL,
    PAYMENT_METHOD     VARCHAR(32)  NULL,
    IDEMPOTENCY_KEY    VARCHAR(64)  NOT NULL,
    APPROVAL_NUMBER    VARCHAR(64)  NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE       DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY         VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY   VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED            BOOLEAN      DEFAULT FALSE,
    DELETED_DATE       DATETIME     NULL,
    DELETED_BY         VARCHAR(255) NULL,

    -- 인덱스
    UNIQUE INDEX idx_point_history_idempotency_key (IDEMPOTENCY_KEY),
    INDEX idx_point_history_owner_id (OWNER_ID)
);

-- NOTIFICATIONS
CREATE TABLE NOTIFICATIONS
(
    NOTIFICATION_ID    BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- NotificationTable 특화 컬럼들
    RECIPIENT_ID       BIGINT       NOT NULL,
    TYPE               VARCHAR(32)  NOT NULL,
    ACTOR_ID           BIGINT       NOT NULL,
    POST_ID            BIGINT       NOT NULL,
    COMMENT_ID         BIGINT       NOT NULL,
    IS_READ            BOOLEAN      DEFAULT FALSE,

    -- BaseTable 공통 컬럼들
    CREATED_DATE       DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY         VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY   VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED            BOOLEAN      DEFAULT FALSE,
    DELETED_DATE       DATETIME     NULL,
    DELETED_BY         VARCHAR(255) NULL,

    -- 인덱스
    INDEX idx_notification_recipient_id (RECIPIENT_ID)
);

-- COMMUNITY COMMENT NOTIFICATION SETTINGS
CREATE TABLE COMMUNITY_COMMENT_NOTIFICATION_SETTINGS
(
    COMMENT_NOTIFICATION_SETTING_ID BIGINT AUTO_INCREMENT PRIMARY KEY,

    -- CommentNotificationSettingTable 특화 컬럼들
    MEMBER_ID          BIGINT       NOT NULL,
    POST_ID            BIGINT       NOT NULL,

    -- BaseTable 공통 컬럼들
    CREATED_DATE       DATETIME     DEFAULT CURRENT_TIMESTAMP,
    CREATED_BY         VARCHAR(255) DEFAULT 'MEET_AGAIN',
    LAST_MODIFIED_DATE DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    LAST_MODIFIED_BY   VARCHAR(255) DEFAULT 'MEET_AGAIN',
    DELETED            BOOLEAN      DEFAULT FALSE,
    DELETED_DATE       DATETIME     NULL,
    DELETED_BY         VARCHAR(255) NULL,

    -- 인덱스
    UNIQUE INDEX idx_comment_noti_setting_member_post (MEMBER_ID, POST_ID)
);
