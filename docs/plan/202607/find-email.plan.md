...
- **Q6 → SMS 인증 필수 + 전체 이메일 반환 (2026-07-12 추가 확정, Q2 마스킹 결정을 대체)**: 지식 기반(name+phone을 아는 것)만으로 이메일을 노출하지 않고, 휴대폰 점유 인증(기존 /api/sms/verification-code → /confirm 흐름의 confirmed 상태)을 선행 요구. 미인증 호출은 `SmsNotVerifiedException` → 400(INVALID_INPUT_VALUE), SignUpValidator 선례와 동일한 배치(Validator 컴포넌트). 단 예외 인자는 **의도적으로 다름**: FindEmailValidator는 원본 phone 대신 `phoneNumber.masked()`를 넘긴다(신규 코드의 PII 로그 최소화 원칙 — 버그 아님, SignUpValidator의 raw 전달을 마스킹으로 통일할지는 별도 일괄 재검토 사안). 점유가 증명되므로 마스킹 불필요 — 전체 이메일 반환(`Email.masked()` 제거). 검증 순서: ① bean validation(400) → ② SMS 인증(400) → ③ 조회(404). 인증 검사는 서비스 분기 금지 원칙에 따라 도메인 Validator 컴포넌트로 배치. find-password도 동일 흐름 공유 예정.
