...
- **Q6 → SMS 인증 필수 + 전체 이메일 반환 (2026-07-12 추가 확정, Q2 마스킹 결정을 대체)**: 지식 기반(name+phone을 아는 것)만으로 이메일을 노출하지 않고, 휴대폰 점유 인증(기존 /api/sms/verification-code → /confirm 흐름의 confirmed 상태)을 선행 요구. 미인증 호출은 `SmsNotVerifiedException` → 400(INVALID_INPUT_VALUE). SMS 인증 검사는 공용 `SmsVerificationValidator`로 추출(SignUpValidator의 중복 checkSmsVerification을 대체 — SignUpValidator·FindEmailService 양쪽이 합성). 예외 인자는 `phoneNumber.masked()`로 **양 경로 통일**(PII 로그 최소화 — 회원가입 경로도 raw→masked로 변경, dataMessage는 로그 전용이라 API 무영향). 점유가 증명되므로 마스킹 불필요 — 전체 이메일 반환(`Email.masked()` 제거). 검증 순서: ① bean validation(400) → ② SMS 인증(400) → ③ 조회(404). 인증 검사는 서비스 분기 금지 원칙에 따라 도메인 Validator 컴포넌트로 배치. find-password도 동일 흐름 공유 예정.
