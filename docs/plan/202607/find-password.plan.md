...
[POST /api/auth/find-password]  (permitAll)
        │  FindPasswordRequest { email, name, phone, newPassword }  (raw String, VO 생성 없음)
        ▼
FindPasswordApi ─────────────────────────────────  boot/ma-boot-web
        │  resetPassword(email, name, phone, newPassword)
        ▼
FindPasswordService (@Service @Transactional) ────  domain/ma-domain-core (application)
...
| 7 | `domain/.../auth/application/FindPasswordService.kt` | 신규 Service (`@Service @Transactional`) — 위 흐름 ①~⑤ 오케스트레이션 | 신규 |
| 8 | `boot/.../auth/api/request/FindPasswordRequest.kt` | 신규 Request DTO — email/name/phone/newPassword + Bean Validation | 신규 |
| 9 | `boot/.../auth/api/FindPasswordApi.kt` | 신규 컨트롤러 — `POST /find-password`, 204, Service 위임 | 신규 |
...
- Service: `FindPasswordService.resetPassword(email: String, name: String, phone: String, newPassword: String)`
- 컨트롤러: `findPassword(@Valid @RequestBody request: FindPasswordRequest)` → 204 (Unit 반환 + `@ResponseStatus(NO_CONTENT)`)
...
- `FindPasswordService` 신규. flat 위임만(분기·private 헬퍼 금지): VO 생성 → validate → findOne → encode → updatePassword.
- 변경 파일: #7.

### Step 4: 웹 계층 (boot)
- `FindPasswordRequest`(raw String만, VO 생성 금지) → `FindPasswordApi`(Service만 의존, 204) → `SecurityConfig` permitAll.
- 변경 파일: #8, #9, #10.
...
- **트랜잭션 경계**: `FindPasswordService`는 쓰기(updatePassword) 포함 → 클래스 단위 `@Transactional`(find-email·sign-up과 동일 선언 방식). 느린 외부호출 없음 → `TransactionTemplate` 불필요.
- **DDL 무변경**: `MemberTable.password` 컬럼 기존재. 스키마 변경·FK 없음.

## 테스트 전략 (/tdd 파이프라인에서 별도 진행 — 시나리오 목록만)

- **API 슬라이스 테스트** (`FindPasswordApiTest`, `@WebMvcTest` + `@MockkBean FindPasswordService`, REST Docs 스냅샷):
...
- **E2E 통합 테스트** (`FindPasswordIntegrationTest`, `@SpringBootTest` + `@AutoConfigureMockMvc`, 실제 DB+embedded Redis, `SmsRepository.confirmVerificationCode`로 인증 셋업 — find-email 통합테스트 그대로 미러링):
